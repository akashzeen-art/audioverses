...
interface CzechSectionProps {
  onClick: () => void
}

export default function CzechSection({ onClick }: CzechSectionProps) {
  const totalHours = czechAudio
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between mb-3 sm:mb-4 flex-wrap gap-2">
        <h3 className="text-lg sm:text-xl font-bold text-[#EAEAEA]">Czech Audio</h3>
        <div className="bg-[#FFD369] text-[#1E1E2F] px-2 sm:px-3 py-1 rounded-full text-xs sm:text-sm font-medium">
          {czechAudio.length} titles
        </div>
      </div>
      <p className="text-[#EAEAEA]/70 text-xs sm:text-sm mb-3 sm:mb-4 leading-relaxed">
        Experience Czech literature and storytelling through our curated Czech audio verse collection
      </p>
